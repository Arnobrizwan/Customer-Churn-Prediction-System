...
def load_model(filename):
    try:
        ...
        st.error(f"File {filename} not found. Please ensure all required models are available.")
        return None
...
def make_predictions(input_df, input_dict):
  ...
  if naive_bayes_model:
      probabilities['Naive Bayes'] = naive_bayes_model.predict_proba(input_df)[0][1]
  if decision_tree_model:
      probabilities['Decision Tree'] = decision_tree_model.predict_proba(input_df)[0][1]

  avg_probability = np.mean(list(probabilities.values()))

  col1, col2 = st.columns(2)  # Make sure this indentation level is correct
...
if selected_customer_option:
    ...
    input_df, input_dict = prepare_input(credit_score, location, gender, age, tenure, balance, num_products, has_credit_card, selected_customer['IsActiveMember'], estimated_salary)
